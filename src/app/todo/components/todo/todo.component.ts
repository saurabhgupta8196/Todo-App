...
export class TodoComponent implements OnInit, OnDestroy {
  ...
  constructor(private _todoService: TodoService, private _notificationService: NotificationService, private _dialog: MatDialog) {
  ...
  public createTodoItem(form): void {
    this._subscriptions.add(
      this._todoService.createTodo(form.todoForm).subscribe(() => {
        this._notificationService.notify('Successfully created an new todo item');
        this.ngOnInit();
      })
    );
  }
  ...
  public taskStatusChange(selectedTodoItem: ITodo): void {
    ...
    this._subscriptions.add(
      this._todoService.updateTodo({ ...selectedTodoItem, done }).subscribe(() => {
        if (!done) this._notificationService.notify(`Todo item is ${done ? 'done at ' + done : 'added'}`);
        this.ngOnInit();
      })
    );
  }
